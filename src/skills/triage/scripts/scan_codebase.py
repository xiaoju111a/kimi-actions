...
CODE_EXTENSIONS = {
    ...
    '.kt', '.scala', '.vue', '.svelte', '.md', '.yml', '.yaml'
}
...
SKIP_DIRS = {
    ...
    '.pytest_cache', '.ruff_cache', 'htmlcov', '.tox', 'egg-info'
}
...
STOP_WORDS = {
    ...
    'using', 'use', 'used', 'want', 'like', 'get', 'set',
    'new', 'add', 'added', 'remove', 'removed', 'change', 'changed'
}
...
def extract_keywords(title: str, body: str) -> List[Tuple[str, int]]:
    ...
    files = re.findall(r'\b([\w/]+\.(?:py|js|ts|tsx|jsx|go|rs|java|rb|yml|yaml|json|md))\b', text_lower)
    for f in files:
...
def search_files(keywords: List[Tuple[str, int]], root_dir: str, max_results: int = 5) -> Dict[str, List[Dict]]:
                ...
                if keyword.lower() in filename or filename in keyword.lower():
                    ...
                    if rel_path not in all_matched_files:
                        matches.append({
                            'file': rel_path,
                            'keyword': keyword,
                            'priority': priority + 1  # Boost priority for filename match
                        })
...
def aggregate_files(file_results: Dict[str, List[Dict]], max_files: int = 10) -> List[Dict]:
    ...
    for keyword, matches in file_results.items():
        for match in matches:
            ...
            file_scores[filepath]['max_priority'] = max(
                file_scores[filepath]['max_priority'], 
                priority
            )
...
def main():
    ...
    summary_parts = []
    if total_files > 0:
        summary_parts.append(f"Found {total_files} relevant files matching {len(file_results)} keywords.")
