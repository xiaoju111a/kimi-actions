...
class Improve(BaseTool):
    ...
    def run(self, repo_name: str, pr_number: int, **kwargs) -> str:
        ...
        skill_instructions = (
            skill.instructions if skill else "Provide code improvement suggestions."
        )
            ...
            try:
                ...
                if inline and suggestions:
                    summary = self._format_summary(
                        suggestions, len(suggestions), included_chunks, command_quote
                    )
                    posted_count = self._post_inline_comments(
                        repo_name, pr_number, suggestions, summary
                    )
                    if posted_count > 0:
                ...
                summary = self._format_summary(
                    suggestions, posted_count, included_chunks, command_quote
                )
                return summary
    ...
    async def _run_agent_improve(
        self, work_dir: str, diff: str, skill_instructions: str, num_suggestions: int
    ) -> str:
        ...
        files_reviewed = (
            ...
            if included_chunks
                ...
                set(
                    ...
                    if s.get("relevant_file")
                )
            )
        )
        lines.append(
            f"Kimi analyzed {files_reviewed} changed files and generated {inline_count} improvement suggestions.\n"
        )
        ...
        if suggestions:
            ...
            if len(suggestions) > 5:
                ...
                for s in suggestions[5:]:
                    ...
                    file_name = s.get("relevant_file", "unknown")
                    summary = (
                        s.get("one_sentence_summary", "").replace("\n", " ").strip()
                    )
                    lines.append(f"- {icon} `{file_name}`: {summary}")
                lines.append("\n</details>")
            lines.append("")
